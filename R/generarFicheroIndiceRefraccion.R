generarFicheroIndiceRefraccion <- function(valoresREF_AERONET, fechaMedida) {
  ...
  ficheroINPUT_REF <- paste(paste('indicesRef', format(fechaMedida, '%Y%m%d_%H%M'), sep = '_'), 'INP', sep = '.')
  ...
  dirINPUT_REF <- file.path(dirFiles, ficheroINPUT_REF, fsep = .Platform$file.sep)
